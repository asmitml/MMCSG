...
Encoder_Cls_dim = 768
Num_labels = 3
intent_column = "encoded_merged_intent"
...
def preprocess_dataset(dataset):
    ...
    model_inputs['intent_labels'] = torch.tensor(dataset[intent_column], dtype=torch.long).to(DEVICE)
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        input_ids, attention_mask, acoustic_input, visual_input, personality_input, labels, intent_labels = batch
        optimizer.zero_grad()
...
def get_val_scores(model,
    ...
    if "Validation" in desc:
        ...
        file_name = RESULT_OUTPUT_DIR + "val/IPC_mmcs" + str(epoch+1) + "_val_results.csv"
        ...
        file_name = RESULT_OUTPUT_DIR + "test/IPC_mmcs" + str(epoch+1) + "_test_results.csv"
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        val_loss , val_acc = val_epoch(model,
                             val_data_loader, 
                             optimizer)
        ...
        print("Epoch: {}\ttrain_loss: {}\tval_loss: {}\tmin_validation_loss: {}\ttrain_acc: {}\tval_acc: {}".format(epoch+1, train_loss, val_loss, min(val_losses), train_acc, val_acc))
...
if __name__ == "__main__":
    ...
    NewDataset_path = './OurData/mmcs_final_with_pf.json'
